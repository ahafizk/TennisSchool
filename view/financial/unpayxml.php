...
$rfinancial = new Rfinancial();
	...
	$count = $rfinancial->get_total_unpaid();
	...
	$result = $rfinancial->get_unpaid_list($start,($start+$limit),$sidx ,$sord);
   ...
   // we should set the appropriate header information 
	...
	{ 
  		...
  		//$total+=$row['total'];
  		echo "<row id='". $start."'>"; 
		echo "<cell><![CDATA[". $result['user_id'][$i]."]]></cell>";
		...
		echo "<cell><![CDATA[". $result['tution_fees'][$i]."]]></cell>";
		echo "<cell><![CDATA[". $result['vat'][$i]."]]></cell>";
		echo "<cell><![CDATA[". $result['pay_for'][$i]."]]></cell>";
		echo "<cell><![CDATA[". $result['collection_date'][$i]."]]></cell>";
		echo "</row>";
		$start++;
   }//end of while 
