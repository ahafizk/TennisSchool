...
require_once('../../control/financial/cupdatepayment.php');
session_start();
if($_SESSION['auths']!=1)
{
        ...
        <a href="#" class=" active"><span class="l"></span><span class="r"></span></a> </div>
			...
			<div id="navbalk">
        		...
        		<br/>
                  ...
                  <form name="form1" action="updatepayment1.php" method="post">
                  <div class="content_body" align="left">
                  
                  <table cellpadding="5" cellspacing="2">
                  <tr>
                  <td class="input_level">Select Student:</td>
                  <td><select id="h" class="input_txt" name="user_id"  >
                  <?php
				  for($i=0;$i<$cnt;$i++)
				  {
					  $name = $record['initial'][$i]." ".$record['call_name'][$i]." ".$record['middle_name'][$i]." ".$record['last_name'][$i];
					
                  	echo "<option value =\"".$record['user_id'][$i]."\">".$name."</option>";
				  }
				  ?>
                  </select>
                  </td>
