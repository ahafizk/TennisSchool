...
if(isset($_POST['Search']))
 {
	...
	if(!$flag)
	{
		...
		forward('../../view/student/searchstudent.php?s=m');
	}
	else 
	{
		...
		else 
		{
			$user_id = $user['id'][0];
			$result = array();
			$result = $rstudent->get_user_temp($user_id);
			if($result<=0)
			{
				$result = $rstudent->get_user_perm($user_id);
			}
			$info = $rstudent->user_group_lesson_info($result);//
			$_SESSION['info']=$info;
			$group_info = $rstudent->get_groups_info();
		}
	}
 }//end of if
else{
    //show profile
    $rstudent = new Rstudent();
    $ret = $rstudent->get_user_profile($_SESSION['USER']);
//    print_r($res);
} 
?>