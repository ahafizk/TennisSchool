...
session_start();
 if(isset($_POST['Update']))
 {
	...
	if(($dat['year']==0||$dat['month']==0||$dat['day']==0)||(strcasecmp($email,'')&&!isValidEmail($email)))
	{
		...
		$_SESSION['invalid'] .= "! Update failed!";
		forward('../../view/student/searchstudent.php');
	}
	else 
	{
		$_SESSION['invalid'] ="";
		...
		$rstudent->update_user($initial,$call_name,$middle_name,$last_name, $gender,$date,$comment,$user_id);
		
		$rstudent->update_address($user_id,$street, $house_number,$home_palate,$post_code,$telephone,$mobile,$email,$fax);	
		$rstudent->update_sport_info($user_id,$knl_id, $bond_number,$skill_single,$skill_double,"","",0,0,0,"");			
