...
if(isset($_POST['next']))
{
	...
	$_SESSION['user_name'] = $record['user_name'][0];
		
		$_SESSION['password'] = $record['password'][0];
		$_SESSION['initial']= $record['initial'][0];
		$_SESSION['call_name'] = $record['call_name'][0];
		$_SESSION['middle_name'] = $record['middle_name'][0];
		$_SESSION['last_name'] = $record['last_name'][0];
		$_SESSION['gender'] = $record['gender'][0];
		$_SESSION['date'] = $record['birth_date'][0];
		$_SESSION['street'] = $record['street'][0];
		$_SESSION['house_number'] = $record['house_number'][0];
		$_SESSION['post_code'] = $record['post_code'][0];
		$_SESSION['location'] = $record['location'][0];
		$_SESSION['telephone'] = $record['phone'][0];
		$_SESSION['mobile'] = $record['mobile'][0];
		$_SESSION['email'] = $record['email'][0];
		$_SESSION['fax'] = $record['fax'][0];
		$_SESSION['user_id']= $record['id'][0];
		//print_r($record);
}
 else if(isset($_POST['save']))
 {
	...
	$dat = date_parse($date);
	if(($dat['year']==0||$dat['month']==0||$dat['day']==0)||(strcasecmp($email,'')&&!isValidEmail($email))||strcmp($password,$repassword))
	{
		...
		$_SESSION['invalid'] .= "!";
		//echo "hello";
		forward('../../view/authorization/changeuser1.php');
	}
	else 
	{
		$_SESSION['invalid'] ="";
		...
		$rauth->update_user("adminuser",$coloums,$condition);
